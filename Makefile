...
lint:
	ruff check --fix --unsafe-fixes {{cookiecutter.package_name}} tests
...
cov:
	coverage run --source {{cookiecutter.package_name}} -m pytest tests --junit-xml=test_result.xml
	coverage report -m
	coverage html
	coverage xml

cov-basic:
	coverage run --omit={{cookiecutter.package_name}}/scripts/*,*/__init__.py,{{cookiecutter.package_name}}/__main__.py --source {{cookiecutter.package_name}} -m pytest tests/unit tests/integration --junit-xml=test_result.xml
	coverage report -m
	coverage html
	coverage xml
